...
# Model
# =========================
model = YOLO("/home/malte/Downloads/Studienprojekt-master/Studienprojekt-master/best.pt")
...
def place_modules_on_line(r, main_box, groin_line_coords, module_map):
    ...
    color_map = {
        ...
        "gbig ray module": (128, 128, 128),      # Gray
        "small gray module": (128, 128, 128),     # alternate naming
        "35mm": (0, 128, 128), 
    }
    ...
    for label, (rel_x_start, rel_x_end, y_thresh) in module_map.items():
        ...
        if r.boxes is not None:
            for box in r.boxes:
                ...
                module_center_y = (y1 + y2) // 2
    

                if (
                    zone_x1 <= module_center_x <= zone_x2 and
                    zone_y1 <= module_center_y <= zone_y2
                ):
                ...
                # Draw marker
                cv2.circle(annotated_frame, (module_center_x, module_center_y), 7, status_color, -1)
                cv2.putText(
...
module_layouts = {
    "v1": {
        "small gray module": (leftv1, leftv1 + lgm_size, 60),
        "yellow module":     (leftv1 + lgm_size,
                              leftv1 + lgm_size + ym_size, 60),
        "Blue Module":       (leftv1 + lgm_size + ym_size,
                              leftv1 + lgm_size + ym_size + bm_size, 60),
        "big gray module":      (leftv1 + lgm_size + ym_size + bm_size,
                              leftv1 + lgm_size + ym_size + bm_size + gm_size, 60)
    },

    "v2": {
        "35mm": (rechtsv2 - mm_size, rechtsv2, 60),
        "small gray module": (rechtsv2 - mm_size - lgm_size, rechtsv2 - mm_size, 60),
    }
}
module_positions = module_layouts["v2"]
